...
Ec::Ec (Pd *p, Fpu *f, Utcb *u, unsigned c, mword e, mword a, mword s, void (*x)()) : Kobject (Kobject::Type::EC, x ? Kobject::Subtype::EC_GLOBAL : Kobject::Subtype::EC_LOCAL), cpu (c), evt (e), pd (p), fpu (f), utcb (u), cont (x)
{
    ...
    pd->Space_mem::update (a, Buddy::ptr_to_phys (utcb), 0, Paging::Permissions (Paging::R | Paging::W | Paging::U), Memtype::Index::MEM_WB, Memtype::Shareability::INNER);
}
...
void Ec::root_invoke()
{
    ...
    for (unsigned i = 0; i < c; i++, p++) {
        ...
        if (p->type == 1) {
            ...
            for (unsigned o; size; size -= 1UL << o, phys += 1UL << o, virt += 1UL << o)
                current->pd->Space_mem::update (virt, phys, (o = static_cast<unsigned>(min (max_order (phys, size), max_order (virt, size)))) - PAGE_BITS, perm, Memtype::Index::MEM_WB, Memtype::Shareability::INNER);
        }
    }
    ...
    current->pd->Space_mem::update (HIPB_ADDR, Buddy::ptr_to_phys (&PAGEH), 0, Paging::Permissions (Paging::R | Paging::U), Memtype::Index::MEM_WB, Memtype::Shareability::INNER);
