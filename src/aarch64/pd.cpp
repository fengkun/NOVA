...
// XXX: Check if src + 1<<ord is not larger than max
void Pd::update_mem_space (Pd *pd, mword src, mword dst, unsigned ord, unsigned pmm, Memtype::Index mt, Memtype::Shareability sh, Space::Index si)
{
    unsigned o;
    ...
    for (mword ssel = src, dsel = dst; ssel < src + (1UL << ord); ssel += (1UL << o), dsel += (1UL << o)) {
        ...
        Hpt::OAddr p;
        Memtype::Index src_mt;
        Memtype::Shareability src_sh;

        auto pm = Paging::Permissions (pd->Space_mem::lookup (s, p, o, src_mt, src_sh) & (Paging::U | pmm));
        ...
        Space_mem::update (d, p, o, pm, mt, sh, si);
    }
