...
void Ec::sys_ctrl_pd()
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s SPD:%#lx DPD:%#lx ST:%u SI:%u SRC:%#lx DST:%#lx ORD:%u PMM:%#x MT:%u SH:%u", static_cast<void *>(current), __func__, r->spd(), r->dpd(), static_cast<unsigned>(r->st()), static_cast<unsigned>(r->si()), r->src(), r->dst(), r->ord(), r->pmm(), static_cast<unsigned>(r->mt()), static_cast<unsigned>(r->sh()));
    ...
    switch (r->st()) {
        ...
        case Space::Type::MEM:
            dst->update_mem_space (src, r->src(), r->dst(), r->ord(), r->pmm(), r->mt(), r->sh(), r->si());
            break;
...
void Ec::sys_ctrl_ec()
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s EC:%#lx", static_cast<void *>(current), __func__, r->ec());
    ...
    if (!(ec->hazard & HZD_RECALL)) {
        ...
        if (Cpu::id != ec->cpu && Ec::remote_current (ec->cpu) == ec)
            Interrupt::send_sgi (Interrupt::Sgi::RKE, ec->cpu);
    }
...
void Ec::sys_ctrl_sc()
{
    ...
    r->set_time_ms (Timer::ticks_to_ms (sc->time));
...
template void Ec::sys_finish<Sys_regs::ABORTED>();
